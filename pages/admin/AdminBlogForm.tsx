...
const AdminBlogForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) navigate('/acesso-a7f9k2/blog');
    } catch (error) {
  ...
  return (
    <>
      <SEO title={isEditing ? 'Editar Artigo' : 'Novo Artigo'} />
          ...
          <main className="p-8">
            <form onSubmit={handleSubmit} className="max-w-4xl mx-auto bg-white rounded-3xl p-10 shadow-sm border border-slate-100">
               <div className="space-y-6">
                ...
                <textarea name="content" value={formData.content} onChange={handleChange} required rows={10} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:border-[#e84c5c] outline-none transition font-medium" />
                <div className="flex justify-end space-x-4">
                  ...
                  <button type="submit" disabled={saving} className="px-10 py-3 bg-[#1a1a3a] text-white rounded-xl font-bold text-[10px] uppercase tracking-widest shadow-xl hover:bg-[#e84c5c] transition-all">Salvar</button>
                </div>
              </div>
            </form>
          </main>
